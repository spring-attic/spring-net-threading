	...
	public class ArrayBlockingQueueTests : BaseThreadingTestCase
	{
		...
		private class AnonymousClassRunnable2 : IRunnable
		{
			...
			public virtual void Run()
			{
				try
				{
					q.Put(new Object());
					q.Put(new Object());
					Assert.IsFalse(q.Offer(new Object(), Delays.Short));
					q.Offer(new Object(), Delays.Long);
					Assert.Fail("should throw an exception");
				}
				catch (ThreadInterruptedException)
				{
		...
		private class AnonymousClassRunnable5 : IRunnable
		{
			...
			public virtual void Run()
			{
				try
				{
					...
					for (int i = 0; i < DEFAULT_COLLECTION_SIZE; ++i)
					{
                        if ( _q.Poll(Delays.Short, out j))
                        {
						    Assert.AreEqual(i, j);
                        }
					}
					Assert.IsFalse(_q.Poll(Delays.Short, out j));
				}
				catch (ThreadInterruptedException)
				{
				}
		...
		private class AnonymousClassRunnable6 : IRunnable
		{
			...
			public virtual void Run()
			{
				...
				{
				    object obj;
					Assert.IsFalse(q.Poll(Delays.Short, out obj));
					Assert.IsNull(obj);
					q.Poll(Delays.Long, out obj);
					q.Poll(Delays.Long, out obj);
					Assert.Fail("should throw an exception");
				}
				catch (ThreadInterruptedException)
		...
		private class AnonymousClassRunnable7 : IRunnable
		{
			...
			public virtual void Run()
			{
				...
				try
				{
					Assert.IsTrue(q.Offer(three, Delays.Medium));
		...
		private class AnonymousClassRunnable9 : IRunnable
		{
			...
			public virtual void Run()
			{
				...
				try
				{
					Assert.IsTrue(q.Poll(Delays.Medium, out i));
		...
		public void TimedPoll()
		{
			...
			for (int i = 0; i < DEFAULT_COLLECTION_SIZE; ++i)
			{
			    q.Poll(Delays.Short, out j);
				Assert.AreEqual(i, j);
			}
			Assert.IsFalse(q.Poll(Delays.Short, out j));
		}
		...
		public void TimedPollWithOffer()
		{
			...
			Thread.Sleep(Delays.Small);
			Assert.IsTrue(q.Offer(zero, Delays.Short));
			t.Interrupt();
			t.Join();
		}
