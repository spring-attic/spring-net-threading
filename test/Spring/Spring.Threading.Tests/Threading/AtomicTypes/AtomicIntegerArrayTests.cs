...
namespace Spring.Threading.AtomicTypes
{
	...
	public class AtomicIntegerArrayTests : BaseThreadingTestCase
	{
		...
		[Test] public void  Constructor2NPE()
		{
			int[] a = null;
		...
		[Test] public void  CompareAndSetInMultipleThreads()
		{
		...
		[Test] public void  GetAndDecrement()
		{
			AtomicIntegerArray ai = new AtomicIntegerArray(DEFAULT_COLLECTION_SIZE);
			for (int i = 0; i < DEFAULT_COLLECTION_SIZE; ++i)
			{
		...
		[Test] public void  GetAndIncrement()
		{
			AtomicIntegerArray ai = new AtomicIntegerArray(DEFAULT_COLLECTION_SIZE);
			for (int i = 0; i < DEFAULT_COLLECTION_SIZE; ++i)
			{
		...
		[Test] public void  CountingInMultipleThreads()
		{
			AtomicIntegerArray ai = new AtomicIntegerArray(DEFAULT_COLLECTION_SIZE);
			for (int i = 0; i < DEFAULT_COLLECTION_SIZE; ++i)
				ai[i] = COUNTDOWN;
