...
{
	/// <summary>
	/// An unbounded <see cref="IBlockingQueue"/> of
	/// <see cref="IDelayed"/> elements, in which an element can only be taken
	/// when its delay has expired. 
	/// </summary>
	/// <author>Doug Lea</author>
	/// <author>Griffin Caprio (.NET)</author>
	public class DelayQueue : AbstractQueue, IBlockingQueue
	{
		[NonSerialized] private object lockObject = new object();

		private PriorityQueue _queue = new PriorityQueue();

		/// <summary>
		/// Creates a new, empty <see cref="DelayQueue"/>
		/// </summary>
		public DelayQueue() {}

		/// <summary>
		///Creates a <see cref="DelayQueue"/> initially containing the elements of the
		...
		public DelayQueue( ICollection collection )
		{
			AddAll( collection );
		}
		...
		public int DrainTo( ICollection collection )
		{
			...
			lock ( lockObject )
			{
				...
				for ( ;; )
				{
					...
					CollectionUtils.Add( collection, _queue.Poll() );
					++n;
				}
				if ( n > 0 )
				{
		...
		object IQueue.Poll()
		{
			// TODO:  Add DelayQueue.Spring.Collections.IQueue.Poll implementation
			return null;
		}
		...
		{
			// TODO:  Add DelayQueue.Spring.Collections.IQueue.Offer implementation
			return false;
		}
		...
		public override void CopyTo( Array targetArray, Int32 index )
		{
			...
			lock ( lockObject )
			{
				...
				int k = 0;
				foreach ( IDelayed currentItem in _queue )
				{
