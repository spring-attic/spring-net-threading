...
{
	/// <summary> 
	/// A thread-safe variant of <see cref="ArrayList"/> in which all mutative
	...
	[Serializable]
	public class CopyOnWriteArrayList : IList, ICloneable, ISerializable
	{
		...
		/// <summary>
		/// Gets a value indicating whether the <see cref="CopyOnWriteArrayList"/> is read-only.
		/// </summary>
		...
		/// <summary>
		/// Gets a value indicating whether the <see cref="CopyOnWriteArrayList"/> has a fixed size.
		/// </summary>
		...
		/// <summary>
		/// The array, accessed only via <see cref="CopyOnWriteArrayList.Array"/>. 
		/// </summary>
		[NonSerialized] private volatile object[] _array;
		...
		public CopyOnWriteArrayList( ICollection collection )
		{
			if ( null == collection )
			{
		...
		public CopyOnWriteArrayList( object[] inputArray )
		{
			if ( null == inputArray )
			{
		...
		private void copyToInternalArray( object[] inputArray, int startIndex, int numberOfElementsToCopy )
		{
			...
			object[] newElements = copyOfRange( inputArray, startIndex, limit, typeof ( Object[] ) );
			lock ( this )
			{
				Array = newElements;
			}
		...
		private static int indexOf( object element, Object[] arrayToSearch, int startingIndex, int fence )
		{
			if ( element == null )
			{
		...
		/// </returns>
		private static int lastIndexOf( object element, Object[] elements, int startingIndex )
		{
		...
		public object[] ToArray( object[] destinationArray )
		{
			object[] elements = Array;
			int len = elements.Length;
		...
		/// </returns>
		public bool AddIfAbsent( object element )
		{
			lock ( this )
			{
			...
			lock ( this )
			{
				...
				if ( len != 0 )
				{
					...
					for ( int i = 0; i < len; ++i )
					{
						...
						if ( !CollectionUtils.Contains( collection, element ) )
						{
							temp[newlen++] = element;
						}
					}
		...
		/// </returns>
		public int AddAllAbsent( ICollection collection )
		{
			if ( null == collection ) throw new ArgumentNullException( "collection", "collection cannot be null" ); 
		...
		/// <returns> true if this list changed as a result of the call
		/// </returns>
		public bool AddAll( int index, ICollection collection )
		{
			if (null == collection ) throw new ArgumentNullException( "collection", "collection cannot be null" );
			int numNew = collection.Count;
			lock ( this )
		...
		protected CopyOnWriteArrayList( SerializationInfo s, StreamingContext context )
		{
			...
			{
				elements[i] =
					s.GetValue( "Spring.Threading.Collections.CopyOnWriteArrayListdata" + i, typeof ( object ) );
			}
			Array = elements;
		}
		...
		private object[] copyOfRange( Object[] original, int from, int to, Type newType )
		{
			...
			object[] copy = (Object[]) System.Array.CreateInstance( newType.GetElementType(), newLength );
			System.Array.Copy( original, from, copy, 0, Math.Min( original.Length - from, newLength ) );
			return copy;
		}

		private object[] copyOf( Object[] original, int newLength, Type newType )
		{
			object[] copy = (Object[]) System.Array.CreateInstance( newType.GetElementType(), newLength );
		...
		/// <summary>
		/// Gets and object that can be used to synchronize access to the <see cref="CopyOnWriteArrayList"/> .
		/// </summary>
		...
		/// <see cref="CopyOnWriteArrayList"/> is synchronized (thread-safe).
		/// </summary>
