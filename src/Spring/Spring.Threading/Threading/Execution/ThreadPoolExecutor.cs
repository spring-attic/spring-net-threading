...
namespace Spring.Threading.Execution
{
	...
	///		<dt>Queuing</dt>
	///		<dd>
	///			Any <see cref="Spring.Threading.Collections.IBlockingQueue"/> may be used to transfer and hold
	...
	///					queue is a <see cref="Spring.Threading.Collections.SynchronousQueue"/> 
	...
	///					example a <see cref="Spring.Threading.Collections.LinkedBlockingQueue"/> without a predefined
	...
	///					<see cref="Spring.Threading.Collections.ArrayBlockingQueue"/>) helps prevent resource exhaustion when
	...
	public class ThreadPoolExecutor : AbstractExecutorService, IDisposable
	{
		...
		/// </summary>
		private IBlockingQueue _workQueue;
				...
				try
				{
					switch ( _currentLifecycleState )
					{
						case ThreadPoolState.RUNNING:
						{
							...
							IRunnable r = (IRunnable) _workQueue.Poll( timeout );
							if ( r != null )
							{
								return r;
							}
						...
						{
							IRunnable r = (IRunnable) _workQueue.Poll();
							if ( r != null )
							{
								return r;
							}
		...
		private IRunnable addIfUnderMaximumPoolSize( IRunnable firstTask )
		{
			...
			lock ( _mainLock )
			{
				...
				{
					nextTask = (IRunnable) _workQueue.Poll();
					if ( nextTask == null )
					{
						nextTask = firstTask;
					}
		...
		public ThreadPoolExecutor( int corePoolSize, int maximumPoolSize, TimeSpan keepAliveTime, IBlockingQueue workQueue )
			: this(
		...
		public override IList ShutdownNow()
		{
			bool fullyTerminated = false;
			lock ( _mainLock )
			{
			...
			return new ArrayList( _workQueue );
		}
